...
class CAPSForm_custom(MappingCheckForm):
    ...
    call_soft = SelectField('选择call软件',choices=["astair","CGS"],validate_choice=False, validators=[DataRequired()],default="astair")
    call_params = StringField('call额外参数设置', validators=[Optional(), Length(min=0, max=50)], default="")
...
class CAPSForm_default(MappingCheckForm):
    ...
    call_soft = HiddenField(default="astair")
    call_params = HiddenField(default="")
    submit_ana = SubmitField('开始比对')
