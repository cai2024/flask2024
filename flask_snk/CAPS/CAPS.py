...
include: "cover.smk"
include: "logging.smk"
include: "call_spikein_modify.smk"
...
rule all:
    input:
        ...
        expand("{output_dir}/final_log/{sample}.log",
               output_dir=config['output_dir'], sample=config["sampleList"]),
        expand("{output_dir}/sp_bam/{call_soft}/sp_{sample}_mCtoT_CpG.mods.gz",
               output_dir=config['output_dir'], sample=config["sampleList"],call_soft=config["call_soft"]),



        expand("{output_dir}/sp_bam/{call_soft}/sp_{sample}_mCtoT_all.mods.gz",
               output_dir=config['output_dir'], sample=config["sampleList"],call_soft=config["call_soft"]),
        expand("{output_dir}/de_bam/{call_soft}/de_{sample}_mCtoT_CpG.mods.gz",
               output_dir=config['output_dir'], sample=config["sampleList"],call_soft=config["call_soft"]),
        expand("{output_dir}/de_bam/{call_soft}/de_{sample}_mCtoT_all.mods.gz",
               output_dir=config['output_dir'], sample=config["sampleList"],call_soft=config["call_soft"]),
